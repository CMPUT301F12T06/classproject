...
public class JSONServer {
	...
	/**
	 * This will check the connection to the json server.
	 * 
	 * @param post
	 *            HttpPost object. This will allow the to specify their own url.
	 * 
	...
	/**
	 * Same functionality as the other method but uses the default httpPost.
	 * 
	 * @return
	 */
	...
	/**
	 * This will get the latest task added. This will use ?action=list&n=1 to
	 * only get the latest task. Currently this is mainly used for cleanup in
	 * the JUnit tests but the functionality may be extended in the future.
	 * 
	...
	/**
	 * This will add the task in JSON format to the server using Gson. The task
	 * will first use a post action to send the data to the server. It will then
	 * take that response from the server to obtain the id for the task and
	 * assign it to our Task object. An update method is then called on the task
	 * after we update it with the obtained id.
	 * 
	 * @param task
		...
		try {
			...
			if (entity != null) {
				...
				LOGGER.log(Level.INFO,
						"responseData ID: " + responseData.getId());
	...
	/**
	 * This will retrieve a task with the given id. This is used internally
	 * online and should not be used outside of this class. It is used in
	...
	public Code addSubmission(String taskId, Submission submission) {
		...
		if (newTask == null) {
			LOGGER.log(Level.SEVERE,
					"newTask is null in addsubmission\nTaskid: " + taskId);
			return Code.FAILURE;
		}
	...
	/**
	 * This will remove the task from the server given a specified task id.
	 * 
	 * @param taskId
	...
	public Code deleteTask(Task task) {
		...
		try {
			...
			// TODO: Maybe add some exception handling. Not too sure what to do
			// besides
	...
	/**
	 * This will update a task with a new task and replace it on the server
	 * 
	 * @param taskId
	 *            The id of the task
	 * @param task
	 *            The Task object to replace the current one
	 * 
	 * @return Code.FAILURE or Code.SUCCESS
	 */
